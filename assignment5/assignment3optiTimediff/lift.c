...
lift_type lift_create(void)
{
    ...
    pthread_cond_init(&lift->enter_exit, NULL);
    pthread_cond_init(&lift->new_floor, NULL);
    for(i = 0; i < N_FLOORS; i++) {
      pthread_cond_init(&lift->cond_floor[i], NULL);
    }
...
/*File* file_create(void) {
  file_out = file;
  file = (file_out) malloc(sizeof(file_pointer));
...
static int lift_done_exit(lift_type lift)
{
  int all_passengers_left = 1;
  int i;

  for(i = 0; i < MAX_N_PASSENGERS; i++) {
    if(lift->passengers_in_lift[i].to_floor == lift->floor) {
      all_passengers_left = 0;
      break;
    }
  }
  return all_passengers_left;
}

static int lift_done_entries(lift_type lift)
{
  int i;
  int all_passengers_boarded = 1;

  for(i = 0; i < MAX_N_PERSONS; i++) {
    if(lift->persons_to_enter[lift->floor][i].id != NO_ID) {
      all_passengers_boarded = 0;
      break;
    }
  }
  int lift_full = (n_passengers_in_lift(lift) == MAX_N_PASSENGERS);
  return (all_passengers_boarded || lift_full);
}
...
{
  pthread_mutex_lock(&lift->mutex);
  //pthread_cond_broadcast(&lift->change); //These functions shall unblock threads blocked on a condition variable.
  pthread_cond_broadcast(&lift->new_floor);

  while(!lift_done_exit(lift)) {
    //pthread_cond_wait(&lift->change, &lift->mutex);
    pthread_cond_wait(&lift->enter_exit, &lift->mutex);
  }

  pthread_cond_broadcast(&lift->cond_floor[lift->floor]);

  while(!lift_done_entries(lift)) {
    pthread_cond_wait(&lift->enter_exit, &lift->mutex);
  }
  pthread_mutex_unlock(&lift->mutex);
}
...
static int passengers_wait_for_floor(lift_type lift, int wait_floor)
{
  return !(!lift->moving && lift->floor == wait_floor);
}
...
void lift_travel(lift_type lift, int id, int from_floor, int to_floor)
{
  ...
  pthread_cond_broadcast(&lift->enter_exit);
  while (passenger_wait_for_lift(lift, from_floor)) {
    //pthread_cond_wait(&lift->change, &lift->mutex);
    pthread_cond_wait(&lift->cond_floor[from_floor], &lift->mutex);
  }
  ...
  while (passengers_wait_for_floor(lift, to_floor)) {
    //pthread_cond_wait(&lift->change, &lift->mutex);
    pthread_cond_wait(&lift->new_floor, &lift->mutex);
  }
  leave_lift(lift, id);
  //pthread_cond_broadcast(&lift->change); //Nu kan andra börja jobba
  pthread_cond_broadcast(&lift->enter_exit);
  pthread_mutex_unlock(&lift->mutex);
}
