...
static void *passenger_thread(void *idptr)
{
  ...
  sprintf(buf, "Passenger #%d", id);
	...
	while(1) {
		...
		//printf("time was: %d and %d\n", clock_gettime(CLOCK_MONOTONIC, &timestamp[0]), clock_gettime(CLOCK_MONOTONIC, &timestamp[1]));
		...
		if(iterations == N_ITERATIONS - 1) {
			printf("Time for id %d is done and took %lld micro seconds.\n", id, total_time);
