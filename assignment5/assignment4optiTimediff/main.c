...
struct lift_msg{
  ...
  int from_floor[N_PLANNED_TRIPS];      // Specify source and destion for the LIFT_TRAVEL message.
  int to_floor[N_PLANNED_TRIPS];
	int trip;
};
...
static void lift_process(void)
{
  ...
  int change_direction, next_floor;
	struct lift_msg passenger_list[MAX_N_PERSONS];
      ...
      //        the elevator
			...
			for(i = 0; i < MAX_N_PASSENGERS; i++) {
				if ( Lift->passengers_in_lift[i].to_floor == Lift->floor) {
					...
					int trip = passenger_list[id].trip;
					//printf("trip is: %d and id is: %d\n", passenger_list[id].trip, id);

					//message_send((char *) &reply, sizeof(reply), QUEUE_FIRSTPERSON + Lift -> passengers_in_lift[i].id, 0);
					Lift->passengers_in_lift[i].id = NO_ID;
					Lift->passengers_in_lift[i].to_floor = NO_FLOOR;

					if (trip == N_PLANNED_TRIPS) {
						message_send((char *) &reply, sizeof(reply), QUEUE_FIRSTPERSON + id, 0);
						//printf("skickar meddelande\n");
					}
					else if (trip > N_PLANNED_TRIPS) {
						printf("KNAAAAAAS\n");
						exit(1);
					}
					else {
						//printf("enter floor\n");
						enter_floor(Lift, id, passenger_list[id].from_floor[trip], passenger_list[id].to_floor[trip]);
					}
					(passenger_list[id].trip)++;
...
static void person_process(int id)
{
  ...
  struct lift_msg m;
	...
	long long int trip_times[N_ITERATIONS];
  ...
  while(1){
    ...
    //    Wait a little while
		...
		for (i = 0; i < N_PLANNED_TRIPS; i++) {
			...
			m.to_floor[i] = to_floor;
			m.from_floor[i] = from_floor;
		}
		...
		message_send((char *) &m, sizeof(m), QUEUE_LIFT, 0);
		int length = message_receive(buf, 4096, QUEUE_FIRSTPERSON + id);
		if(length < sizeof(struct lift_msg)) {
			fprintf(stderr, "Message too short! \n");
			continue;
		}
		//sleep(5);
		...
		if (iterations < N_ITERATIONS) {
			trip_times[iterations] = timediff;
		}
		if (iterations == N_ITERATIONS - 1) {
			fprintf(stderr, "ID %d is done, the time it took was %lld microseconds \n", id, total_time);
			FILE *file = fopen("out.csv", "a+");
			fprintf(file, "ID, Trip, time (us)\n");
    ...
    if(!strcmp(message, "new")){
      ...
      //   record the new pid in person_pid[])
			if(current_person_id < MAX_N_PERSONS) {
				...
				si_ui_show_error("Too many persons has been created");
			}

    }
		//Create maximm number o persons
		else if (!strcmp(message, "max")) {
			//FILE *file = fopen("out.csv", "a+");
			//fprintf(file, "ID, Trip, time (us)\n");
			while (current_person_id < MAX_N_PERSONS) {
				person_pid[current_person_id] = fork();
				if (!person_pid[current_person_id]) {
					person_process(current_person_id);
				}
